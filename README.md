...
1. **Isolated Prompts (XML Enveloping)**: Consistent with prompt engineering best practices, all system and task-specific prompts are housed in `server/app/prompts.py` using well-structured XML tags. This separates algorithmic adjustments from software delivery routes.
...
3. **Recursive Character Chunking**: Integrates a custom semantic text splitter that divides documents (PDF, DOCX, TXT) along double newlines, newlines, and spaces to preserve clinical context (default: 800-character chunks with 150-character overlap).
...
5. **Session Persistence Engine**: Leverages custom `localStorage` state rehydration to cache active tabs, interactive modes (Live/Mock), patient case sheets, visual radiograph analyses, compiled SOAP notes, RAG document catalogs, and full chat histories. Data survives page refreshes and browser crashes, with a built-in **"Reset / New Case"** handler to securely flush session cache at any time.
...
Medai is fully optimized for containerized or serverless hosting environments. Both the React/Vite client and the FastAPI backend are deployed using Render's global edge network.

### Backend Web Service Settings (FastAPI)
- **Root Directory**: `server`
- **Runtime**: `Python`
- **Build Command**: `pip install -r requirements.txt`
- **Start Command**: `uvicorn main:app --host 0.0.0.0 --port $PORT`
- **Environment Variables**:
  - `GEMINI_API_KEY`: Your official Gemini API Key.
  - `FRONTEND_URL`: Secure allowed CORS origin (e.g., `https://medai-app-6j28.onrender.com`).

### Frontend Static Site Settings (React/Vite)
- **Root Directory**: `client`
- **Build Command**: `npm run build` (outputs to the static `dist/` directory)
- **Publish Directory**: `dist`
- **Environment Variables**:
  - `VITE_API_URL`: Fully qualified backend edge endpoint (e.g., `https://medai-api-2e7g.onrender.com`).
- **Redirects/Rewrites Rule**:
  - **Source**: `/*`
  - **Destination**: `/index.html`
  - **Action**: `Rewrite` (ensures clean SPA client-side routing on page refreshes).
